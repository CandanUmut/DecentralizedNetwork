...
pub struct Metadata {
    pub tags: Vec<String>,          // Tags associated with the file
    pub category: String,           // Category of the content
    pub timestamp: u64,             // Timestamp for when the file was added
    pub description: Option<String> // Optional description of the file
}
...
impl IPFSHandler {
    ...
    pub async fn add_file(
        ...
        metadata: Option<Metadata>, // Metadata is now optional
    ) -> Result<String> {
        // Ensure metadata is set or default
        let metadata = metadata.unwrap_or_else(|| Metadata {
            tags: vec![],
            category: "Uncategorized".to_string(),
            timestamp: chrono::Utc::now().timestamp() as u64,
            description: None,
        ...
        println!("Serialized metadata: {}", metadata_json);
    
        let metadata_cid = Cid::new_v1(
            cid::Codec::DagCBOR, // Compatible codec for structured data
            Code::Sha2_256.digest(metadata_json.as_bytes()),
        );
    
        // Store metadata in IPFS
        self.ipfs
            .put_block(ipfs::Block::new(
                metadata_json.into_bytes().into_boxed_slice(),
                metadata_cid.clone(),
            ))
            .await?;
        println!("Metadata CID: {}", metadata_cid);
    
        // Step 8: Store metadata linkage
        // Step 8: Link Metadata CID to Root CID
        let linkage_json = serde_json::json!({
            "metadata_cid": metadata_cid.to_string(),
            "original_data_cid": root_cid.to_string(), // Store root CID for tracing
        });

        let linkage_cid = Cid::new_v1(
            cid::Codec::DagCBOR, // Compatible codec for structured data
            Code::Sha2_256.digest(linkage_json.to_string().as_bytes()),
        );

        self.ipfs
            .put_block(ipfs::Block::new(
                linkage_json.to_string().into_bytes().into_boxed_slice(),
                linkage_cid.clone(),
            ))
            .await?;
        println!("Linked Metadata CID to Root CID: {}", metadata_cid);
    
        // Step 9: Pin and announce the root CID
        self.ipfs.insert_pin(&root_cid, true).await?;
        self.ipfs.provide(root_cid.clone()).await?;
        println!("Pinned and announced root CID.");
    
        // Step 10: Return the final CID
    ...
    pub async fn add_message(&self, content: &str) -> Result<String, anyhow::Error> {
        // Step 1: Generate a unique temporary file path
        let temp_file_path = format!("/tmp/message_{}.txt", Uuid::new_v4());

        // Step 2: Write the message content to the temporary file
        ...
        // Step 3: Add the file to IPFS
        let cid_result = self.add_file(&temp_file_path, None, None).await;

        // Step 4: Clean up the temporary file
        if let Err(e) = fs::remove_file(&temp_file_path).await {
            eprintln!("⚠️ Warning: Failed to remove temporary file: {:?}", e);
        } else {
            println!("🧹 Temporary file removed: {}", temp_file_path);
        }

        // Step 5: Return the CID
        cid_result
    }
        ...
        while let Some(entry) = dir_entries.next_entry().await? {
            ...
            if file_type.is_file() {
                ...
                if let Some(file_path) = path.to_str() {
                    ...
                    let cid = self.add_file(file_path, None, None).await?; // Single CID
    ...
    pub async fn retrieve_metadata_cid(&self, file_cid: &Cid) -> Result<Cid> {
        // Fetch the file block from IPFS
        let block = self.ipfs.get_block(file_cid).await.map_err(|e| {
            anyhow::anyhow!("Failed to fetch file block for CID {}: {}", file_cid, e)
        })?;
    
        // Attempt to parse the block as JSON to locate the "metadata_cid"
        let metadata_cid_str = match serde_json::from_slice::<serde_json::Value>(block.data()) {
            Ok(json) => json
                .get("metadata_cid")
                .and_then(|val| val.as_str())
                .ok_or_else(|| anyhow::anyhow!("Missing 'metadata_cid' field in JSON for CID {}", file_cid))?
                .to_string(),
            Err(_) => {
                return Err(anyhow::anyhow!(
                    "Block data for CID {} is not valid JSON for metadata linkage",
                    file_cid
                ));
            }
        };
        ...
        let metadata_cid = metadata_cid_str.parse().map_err(|e| {
            anyhow::anyhow!("Invalid CID format in 'metadata_cid' field: {}", e)
        })?;
    ...
    pub async fn get_file(&self, cid_str: &str, include_metadata: bool) -> Result<(Vec<u8>, Option<Metadata>)> {
        println!("Attempting to fetch file with CID: {}", cid_str);
    
        // Parse the CID string into a Cid object
        let cid: Cid = cid_str.parse().map_err(|e| anyhow::anyhow!("Invalid CID format: {}", e))?;
        println!("Successfully parsed CID: {:?}", cid);
    
        // Retrieve the block from IPFS
        let block = self.ipfs.get_block(&cid).await.map_err(|e| anyhow::anyhow!("Block retrieval failed: {}", e))?;
        println!("Successfully retrieved block for CID: {}", cid_str);
    
        // Decode the file content
        let file_content = block.data().to_vec();
    
        // Optionally retrieve metadata
        let metadata = if include_metadata {
            match self.retrieve_metadata_cid(&cid).await {
                Ok(metadata_cid) => {
                    // Fetch the metadata block using the retrieved metadata CID
                    let metadata_block = self.ipfs.get_block(&metadata_cid).await.map_err(|e| {
                        anyhow::anyhow!("Metadata block retrieval failed: {}", e)
                    })?;
                    
                    // Decode metadata as JSON
                    let metadata_json = String::from_utf8(metadata_block.data().to_vec()).map_err(|e| {
                        anyhow::anyhow!("Invalid metadata encoding: {}", e)
                    })?;
                    Some(serde_json::from_str(&metadata_json).map_err(|e| {
                        anyhow::anyhow!("Metadata deserialization failed: {}", e)
                    })?)
                }
                Err(e) => {
                    eprintln!("Failed to retrieve metadata CID: {}", e);
                    None
                }
            }
        } else {
